% Auto-generated LaTeX for EXP-6b  Selective screening Pareto frontier
% 2026-08-24T18:06:36.823956+00:00

\begin{table}[t]
  \centering
  \caption{Selective-screening Pareto (EXP-6b, \textsc{sim}).
    \textbf{Provenance: fully simulated.}
    Semantic per-call latency from lognormal fitted to EXP-1 LIVE\_BENCHMARK T7
    (Table~7): $P50 \approx 9{,}500$\,ms, $\sigma_{\ln}=0.26$.
    Static base: $\mathcal{N}(3.2\,\text{ms},\,0.8\,\text{ms})$.
    Throughput at rate=1.0 is $3.2\times10^{-4}$ (raw\_CANONICAL.json:
    0.000318); displayed as $3.2\times10^{-4}$ to avoid 3 d.p.\ truncation to 0.}
  \label{tab:exp6b_0}
  \begin{tabular}{rrrr}
    \toprule
    \textbf{Semantic rate} & \textbf{Recall} & \textbf{Throughput (norm.)} & \textbf{Slowdown} \\
    \midrule
    0.00 & 60.0\%  & $1.0$                & $1\times$          \\
    0.05 & 62.5\%  & $1.3\times10^{-2}$   & ${\sim}77\times$   \\
    0.20 & 70.0\%  & $2.0\times10^{-3}$   & ${\sim}500\times$  \\
    0.50 & 80.0\%  & $1.0\times10^{-3}$   & ${\sim}1000\times$ \\
    1.00 & 100.0\% & $3.2\times10^{-4}$   & ${\sim}3100\times$ \\
    \bottomrule
  \end{tabular}
\end{table}
